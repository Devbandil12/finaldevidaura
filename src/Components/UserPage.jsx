...
import { Pencil, Trash2, Plus, MapPin, Star, Heart, FileText, ChevronRight } from 'lucide-react';
import { toast } from "react-toastify";
import { useNavigate } from "react-router-dom";

/*
  UserPage — Redesigned & Restructured
  - Modern, professional responsive layout (TailwindCSS)
  - Sidebar profile, main content tabs, compact cards, order drawer
  - Accessible modal with FocusLock
  - Responsive grid that collapses to single column on small screens
  - All previous features preserved (profile image upload, address add/edit, orders, reviews, queries)
  - Clean componentization inside single file for easy extraction
*/
...
const IconBtn = ({ children, onClick, title = '', className = '' }) => (
  <button onClick={onClick} title={title} className={`p-2 rounded-md hover:bg-gray-100 transition ${className}`}>
    {children}
  </button>
);
...
const FloatingInput = React.forwardRef(({ label, error, className = '', ...props }, ref) => (
  <div className={`relative w-full ${className}`}>
    <input
      ...
      {...props}
    />
    <label className="absolute left-3 -top-2 bg-white px-1 text-slate-500 text-sm transition-all pointer-events-none
...
const FloatingDropdown = ({ label, value, onChange, options = [] }) => {
  ...
  return (
    <div className="relative w-full">
      ...
      {open && (
        <ul role="listbox" className="absolute z-30 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
          {options.map(opt => (
            <li key={opt}
                role="option"
...
const ProfileCard = ({ userdetails, wishlist=[], cart=[], onEdit, onProfileImageChange }) => {
  ...
  const handleRemove = async () => {
    const prev = localUrl; setLocalUrl(null);
    try { await onProfileImageChange(null); toast.success('Removed'); } catch (e){ setLocalUrl(prev); toast.error('Failed remove'); }
  };

  return (
    <aside className="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
      <div className="flex items-center gap-4">
        <div className="w-20 h-20 rounded-full overflow-hidden flex items-center justify-center" style={{background:bg}}>
      ...
      <div className="flex gap-2">
        <button onClick={()=>fileRef.current.click()} className="flex-1 px-3 py-2 rounded-md border text-sm">Change</button>
        <button onClick={handleRemove} className="px-3 py-2 rounded-md border text-sm">Remove</button>
      </div>
      ...
      <div className="mt-2 border-t pt-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-slate-50 p-3 rounded-lg text-center">
            <div className="text-sm text-slate-400">Cart</div>
      ...
      <div className="mt-2">
        <button onClick={onEdit} className="w-full px-4 py-2 bg-slate-900 text-white rounded-md">Edit Profile</button>
      </div>
    </aside>
  );
};
...
export default function UserPage() {
  ...
  // forms
  const { register: regProfile, handleSubmit: submitProfile, reset: resetProfile, setValue: setProfileValue, watch: watchProfile, formState: { errors: profileErrors, isSubmitting: isProfileSubmitting } } = useForm({ defaultValues: { name:'', phone:'', dob:'', gender:'' } });

  const { register: regAddr, handleSubmit: submitAddr, reset: resetAddr, setValue: setAddrValue, watch: watchAddr, formState: { errors: addrErrors, isSubmitting: isAddrSubmitting } } = useForm({ defaultValues: { name:'', phone:'', altPhone:'', address:'', city:'', state:'', postalCode:'', landmark:'', addressType:'' } });

  const productMap = useMemo(()=>{ const m=new Map(); products.forEach(p=>m.set(p.id,p)); return m; }, [products]);
  const findProduct = useCallback(id => productMap.get(id), [productMap]);

  useEffect(()=>{
    if (userdetails) {
      resetProfile({ name:userdetails.name||'', phone:userdetails.phone||'', dob:userdetails.dob||'', gender:userdetails.gender||'' });
      getorders && getorders();
  ...
  useEffect(()=>{ if (editingAddr) resetAddr(editingAddr); }, [editingAddr]);
  ...
  const onProfileSave = async (data) => {
    try { const ok = await updateUser(data); if (ok) { toast.success('Profile updated'); setShowProfileModal(false); } else toast.error('Failed'); } catch(e){toast.error('Error')}
  };

  const onProfileImageChange = async (urlOrNull) => {
    try { const ok = await updateUser({ profileImage: urlOrNull }); if (!ok) toast.error('Failed to save'); }
    catch(e){ toast.error('Error'); }
  };

  const onAddAddress = async (data) => {
    try { const ok = await addAddress(data); if (ok) { toast.success('Added'); setIsAdding(false); resetAddr(); } else toast.error('Failed'); }
    catch(e){ toast.error('Error'); }
  };

  const onEditAddressSave = async (data) => {
    if (!editingAddr) return;
    try { const ok = await editAddress(editingAddr.id, data); if (ok) { toast.success('Updated'); setEditingAddr(null); resetAddr(); } else toast.error('Failed'); } catch(e){ toast.error('Error'); }
  };

  const onDeleteAddress = async (id) => { if (!confirm('Delete?')) return; try{ await deleteAddress(id); toast.success('Deleted'); }catch(e){toast.error('Failed')} };
  const onSetDefault = async (id) => { try{ await setDefaultAddress(id); toast.success('Default set'); }catch(e){toast.error('Failed');} };
    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-12">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4">
          <ProfileCard userdetails={userdetails} wishlist={wishlist} cart={cart} onEdit={()=>setShowProfileModal(true)} onProfileImageChange={onProfileImageChange} />

          <div className="mt-6 bg-white rounded-lg p-4 shadow-sm">
            <h4 className="text-sm font-medium text-slate-700 mb-3">Account Actions</h4>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2">
                {['orders','addresses','reviews','queries'].map(t=> (
                  <button key={t} onClick={()=>setActiveTab(t)} className={`px-3 py-1 rounded-full text-sm ${activeTab===t ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700'}`}>
              ...
              {activeTab==='addresses' && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      <button onClick={()=>{ setIsAdding(v=>!v); setEditingAddr(null); }} className="px-3 py-1 rounded-md border">{isAdding ? 'Close' : 'Add Address'}</button>
                    </div>
                  </div>

                  {isAdding && (
                    <form onSubmit={submitAddr(onAddAddress)} className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4 bg-slate-50 p-4 rounded-md">
                      <FloatingInput label="Full name" {...regAddr('name', { required: 'Name required' })} error={addrErrors.name?.message} />
                      <FloatingInput label="Phone" {...regAddr('phone', { required: 'Phone required', minLength: { value:6, message:'Short' } })} error={addrErrors.phone?.message} />
                      <FloatingInput label="Address" className="md:col-span-2" {...regAddr('address', { required: 'Address required' })} error={addrErrors.address?.message} />
                      <FloatingInput label="City" {...regAddr('city', { required: 'City required' })} error={addrErrors.city?.message} />
                      <FloatingInput label="State" {...regAddr('state')} error={addrErrors.state?.message} />
                      <FloatingInput label="Postal Code" {...regAddr('postalCode', { required: 'Postal code' })} error={addrErrors.postalCode?.message} />
                      ...
                      <div className="md:col-span-2 flex gap-2 mt-2">
                        ...
                        <button type="button" onClick={()=>{ setIsAdding(false); resetAddr(); }} className="px-4 py-2 rounded-md border">Cancel</button>
                      </div>
                    </form>
                  )}

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {address.map(a => <AddressCard key={a.id} addr={a} onEdit={(x)=>{ setEditingAddr(x); setIsAdding(false); }} onDelete={onDeleteAddress} onSetDefault={onSetDefault} />)}
                  </div>

                  {editingAddr && (
                    <div className="mt-4 p-4 bg-slate-50 rounded-md">
                      <h4 className="font-medium mb-2">Edit Address</h4>
                      <form onSubmit={submitAddr(onEditAddressSave)} className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <FloatingInput label="Full name" {...regAddr('name', { required: 'Name required' })} error={addrErrors.name?.message} />
                        <FloatingInput label="Phone" {...regAddr('phone', { required: 'Phone required' })} error={addrErrors.phone?.message} />
                        <FloatingInput label="Address" className="md:col-span-2" {...regAddr('address', { required: 'Address required' })} error={addrErrors.address?.message} />
                        <div className="md:col-span-2 flex gap-2 mt-2">
                          <button type="submit" className="px-4 py-2 bg-slate-900 text-white rounded-md">Update</button>
                          <button type="button" onClick={()=>{ setEditingAddr(null); resetAddr(); }} className="px-4 py-2 rounded-md border">Cancel</button>
                        </div>
                      </form>
                    </div>
                  )}
            ...
            <div className="mt-4">
              {drawerOrder.items?.map(it=> (
                <div key={it.id} className="flex items-center gap-3 py-3 border-b">
                  <img src={findProduct(it.productId)?.imageurl?.[0] || findProduct(it.productId)?.imageurl} alt="" className="w-12 h-12 rounded" />
                  <div>
          ...
          <FocusLock returnFocus autoFocus>
            <div className="bg-white rounded-lg w-full max-w-md p-6">
              ...
              <form onSubmit={submitProfile(onProfileSave)} className="grid gap-3">
                ...
                <FloatingInput label="Phone" {...regProfile('phone', { minLength:{value:6,message:'Short'} })} error={profileErrors.phone?.message} />
