...
// address form "add a new number") — as opposed to CheckoutOtpModal,
// which is specifically for the COD risk-engine-triggered flow at
// checkout. Same visual language, generic copy since the context varies.
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            <button onClick={onClose} disabled={verifying} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:text-black hover:bg-slate-100 transition-colors disabled:opacity-40" aria-label="Close">
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="px-8 pt-10 pb-8 flex flex-col items-center text-center">
              ...
              <div className="mt-7">
                <OtpInput otp={otp} setOtp={setOtp} length={6} onComplete={handleComplete} isSuccess={false} />
              </div>

              <div className="mt-4 h-5">
                {verifying && <div className="flex items-center gap-2 text-xs text-slate-500"><Loader2 className="w-3.5 h-3.5 animate-spin" />Verifying...</div>}
                {!verifying && error && <p className="text-xs font-medium text-rose-500">{error}</p>}
              </div>

              <div className="mt-4 text-xs text-slate-400">
                {secondsLeft > 0 ? <span>Code expires in {mm}:{ss}</span> : <span className="text-rose-500 font-medium">Code expired</span>}
              </div>

              <button onClick={handleResend} disabled={resendCooldown > 0 || resending} className="mt-2 text-xs font-semibold text-slate-700 hover:text-black underline decoration-slate-300 underline-offset-4 disabled:opacity-40 disabled:no-underline transition-colors">
