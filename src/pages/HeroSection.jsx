...
const HeroSection = () => {
  ...
  // 🔴 FIX 1: Fetch Data with Cleanup to prevent "Double Load" flicker
  useEffect(() => {
    let isCancelled = false; // Flag to track if component is unmounted
    ...
    // Cleanup function: Prevents setting state if React unmounts quickly (Strict Mode)
    return () => {
      isCancelled = true;
    };
  }, []);

  // Prepare Variables (Safe to do here)
  ...
  // Hook: Callback (MUST be before any return)
  ...
  // Hook: Layout Effect (MUST be before any return)
  useLayoutEffect(() => {
    // If loading or using full banner, do NOT run split animation logic
    if (isLoading || isFullBanner) return;
    ...
    return () => ctx.revert();
  }, [activeBanner, isLoading, isFullBanner]);

  // ---------------------------------------------------------
  // 2. CONDITIONAL RETURNS (Only safe AFTER all hooks)
  // ---------------------------------------------------------
  
  // Loading State
  if (isLoading) {
    return <div className="h-screen w-full bg-white" />; // Or your Loader component
  }
