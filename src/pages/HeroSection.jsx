...
const HeroSection = () => {
  ...
  useEffect(() => {
    const imagesToLoad = [
      activeBanner?.imageUrl || bottleMain,
      activeBanner?.imageLayer1 || bottleLayer1,
      activeBanner?.imageLayer2 || bottleLayer2
    ].filter(Boolean);
  ...
  // 4. Parallax Effect
  useEffect(() => {
    if (!comp.current || isMobile) {
      if (isMobile) gsap.to([".parallax-layer-back", ".parallax-layer-mid", ".parallax-layer-front"], { x: 0, y: 0, duration: 0.5 });
      return;
    }
  ...
  return (
    <div ref={comp} className="relative w-full min-h-screen text-[#1a1a1a] overflow-hidden selection:bg-[#D4AF37] selection:text-white pb-20 lg:pb-0">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0 flex items-center justify-center opacity-[0.03]">
        <h1 className="text-[19vw] font-bold tracking-tighter text-black whitespace-nowrap parallax-layer-back">DEVID AURA</h1>
      </div>

      <main ref={containerRef} onMouseMove={handleMouseMove} className="relative z-10 w-full max-w-[1800px] mx-auto min-h-screen flex flex-col lg:flex-row items-center px-6 lg:px-16 pt-[80px]">
        ...
        <div className="w-full lg:w-[45%] flex flex-col justify-center items-center lg:items-start text-center lg:text-left z-20 order-2 lg:order-1 mt-12 lg:mt-0">
          ...
          {/* FEATURE GRID: Luxury Typography & Slide-in */}
          {/* FEATURE GRID: Slider on Mobile, Grid on Desktop */}
          <div className="w-full mt-8 lg:mt-4">
            <div className="
    /* Mobile Styles: Horizontal Scroll */
    flex overflow-x-auto snap-x-mandatory no-scrollbar gap-6 pb-4
    /* Desktop Styles: Reset to Grid */
    lg:grid lg:grid-cols-2 lg:overflow-visible lg:gap-y-8 lg:pb-0
  ">
              {features.map((item, index) => (
                <div
                  key={index}
                  className="
          usp-item group flex flex-shrink-0 items-start gap-4 opacity-0
          /* Mobile: Card width and snapping */
          w-[280px] snap-center bg-white/50 p-4 rounded-2xl border border-gray-100
          /* Desktop: Reset width and background */
          lg:w-auto lg:bg-transparent lg:p-0 lg:border-none
        "
                >
                  {/* Icon Container */}
                  ...
                  <div className="text-left flex flex-col justify-center">
                    <h3 className="usp-title text-[11px] mb-1 group-hover:text-[#D4AF37] transition-colors duration-300">
                      {item.title}
                    </h3>
                    <p className="usp-description text-sm opacity-80 leading-snug">
                      {item.desc}
                    </p>
                  </div>
                </div>
            ...
            {/* Mobile-only Scroll Indicator Dot */}
        ...
        {/* RIGHT: Visuals with Darken Blending */}
        <div className="w-full lg:w-[50%] h-[50vh] lg:h-[85vh] lg:max-h-[900px] relative flex items-center justify-center lg:items-center order-1 lg:order-2 perspective-1000 lg:pt-15  lg:pr-12 ml-auto">
          ...
          <div className="hero-image-container relative w-full h-full flex items-center justify-center opacity-0 scale-125">
            {displayData.layer1 && (
              <div ref={layerBackRef} className="absolute top-[6%] left-[6%] lg:left-[10%] w-[140px] lg:w-[220px] pointer-events-none transform rotate-[10deg] z-0">
            ...
            {displayData.layer2 && (
              <div ref={layerMidRef} className="absolute bottom-[16%] right-[6%] lg:right-[10%] w-[120px] lg:w-[180px] pointer-events-none transform -rotate-[8deg] z-0">
